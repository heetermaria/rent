...
rent_growth_rate = county_L_to_W %>% group_by(GEOID) %>% arrange(year) %>% mutate(diff_year = strtoi(year) - lag(strtoi(year)), diff_growth = B25056_001 - lag(B25056_001), rate_percent = (diff_growth/diff_year)/B25056_001 * 100)
# view 
...
rent_price_ratio = rent_growth_rate %>% group_by(GEOID) %>% arrange(year) %>% mutate(rent_price_ratio= B25056_001/B25075_001)
# view 
view(rent_price_ratio)

# get rid of figures missing either rent or housing prices using tidyr method 
rent_price_ratio = rent_price_ratio %>% drop_na(B25056_001) 
rent_price_ratio = rent_price_ratio %>% drop_na(B25075_001)
view(rent_price_ratio)

# add in the price deflator for personal consumption expenditures excluding good & energy to deflate nominal values
...
```{r}
# building a regression of year t to year t + 1 rent growth on year t rent-price ratio 
